...
# Table 6.3
population_means_gender_age <- c(0.042,0.071,0.069,0.063,0.057,0.050,0.023,0.003,0.062,0.113,0.128,0.113,0.089,0.072,0.040,0.007)
# Table 6.7
...
population_means_gender_age <- fn_scale_probs(population_means_gender_age)
...
gender_age <- matrix(rmultinom(nobs, 1, prob = population_means_gender_age), nrow = nobs, ncol = length(population_means_gender_age), byrow = TRUE)
colnames(gender_age) <- c(paste0("M_AGE", 1:(length(population_means_gender_age)/2)), paste0("F_AGE", 1:(length(population_means_gender_age)/2)))
...
# Table A4.4
PCS <- sum(c(0.44622,0.30722,0.35220,0.23948,0.00060,0.02284,-0.18696,-0.28056)*c(PF,RP,BP,GH,VT,SF,RE,MH))
MCS <- sum(c(-0.26540,-0.09193,-0.15782,-0.01826,0.25079,0.22837,0.41798,0.52900)*c(PF,RP,BP,GH,VT,SF,RE,MH))
...
#use_data(phd_data, compress = "xz", overwrite = TRUE)
